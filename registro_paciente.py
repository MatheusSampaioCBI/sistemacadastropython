...
# Função para criar o banco de dados e a tabela de usuários, se necessário
...
def registrar_usuario():
    ...
    cursor.execute("INSERT INTO usuarios (nome, telefone, endereco, cpf, usuario, senha) VALUES (?, ?, ?, ?, ?, ?)",
                       (nome, telefone, endereco, cpf, usuario, senha))
    conn.commit()
    conn.close()
...
tk.Label(frame, text="Telefone:", font=font_label, bg="#caf0f8", fg=label_fg).pack()
telefone_entry = tk.Entry(frame, font=font_entry, bg=entry_bg)
telefone_entry.pack(pady=5)
tk.Label(frame, text="Endereço:", font=font_label, bg="#caf0f8", fg=label_fg).pack()
endereco_entry = tk.Entry(frame, font=font_entry, bg=entry_bg)
